...
function init_commission_table() {
 ...
 initDataTable('.table-commission', admin_url + 'affiliate/report_transaction_table', false, false, fnServerParams);
}
...
var canvas = document.getElementById("commission_chart");
  ...
  $.post(admin_url + 'affiliate/report_transaction_chart/', data).done(function(response) {
     response = JSON.parse(response);
      ...
      colors: [ '#119EFA','#ef370dc7','#15f34f','#791db2d1', '#DDDF00', '#24CBE5', '#64E572', '#FF9655', '#FFF263','#6AF9C4','#50B432','#0d91efc7','#ED561B']
       ...
       Highcharts.chart('commission_chart', {
         ...
         title: {
             text: '<?php echo _l('transaction_chart') ?>'
         },
         subtitle: {
             text: ''
         },
