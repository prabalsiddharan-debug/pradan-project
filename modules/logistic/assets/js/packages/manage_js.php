...
"use strict";
	...
	var table_packages = $('.table-packages');    
	initDataTable('.table-packages', window.location.href, [], [],
	        packageServerParams, [1, 'desc']);

	 $.each(packageServerParams, function(i, obj) {
    ...
    set_hide_column('table-packages', 'table-packages_hide_column', false);
})(jQuery);
...
function assign_driver(invoker){
	...
	$('#addsign_driver_modal input[name="redirect_url"]').val(admin_url+'logistic/packages');
...
function send_package(package_id){
    "use strict";
    $('#send_mail_modal_container').empty();
    $("#send_mail_modal_container").load(admin_url + 'logistic/send_package_modal/' + package_id, function(response, status, xhr) {
          if (status == "error") {
              alert_float('danger', xhr.statusText);
          }
          init_selectpicker();
          $('input[name="redirect_url"]').val(admin_url+'logistic/packages');
