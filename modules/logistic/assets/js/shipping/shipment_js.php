...
$('select[name="invoice_id"]').on('change', function(){
	 ...
	 if(invoice_id != ''){
	 	$.post(admin_url + 'logistic/get_package_info_by_invoice_for_shipping/'+ invoice_id).done(function (response) { 
			response = JSON.parse(response);
...
function add_shipment(){
	...
	"use strict";
	$.post(admin_url + 'logistic/add_shipment_row/'+ shipment_key).done(function (response) {
        response = JSON.parse(response);
...
function view_shipment_file(id, rel_id) {
  "use strict"; 
      ...
      $("#shipment_file_data").load(admin_url + 'logistic/file_shipment/' + id + '/' + rel_id, function(response, status, xhr) {
          if (status == "error") {
...
function delete_shipment_attachment(id) {
  "use strict"; 
    if (confirm_delete()) {
        requestGet('logistic/delete_shipment_attachment/' + id).done(function(success) {
