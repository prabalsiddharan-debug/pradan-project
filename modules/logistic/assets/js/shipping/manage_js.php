...
"use strict";
	...
	var table_shippings = $('.table-shipping');    
	initDataTable('.table-shipping', window.location.href, [], [],
	        shippingServerParams, [1, 'desc']);

	 $.each(shippingServerParams, function(i, obj) {
        $('select' + obj).on('change', function() {  
            table_shippings.DataTable().ajax.reload()
                .columns.adjust();
        });
    });
    ...
    set_hide_column('table-shipping', 'table-shippings_hide_column', false);
})(jQuery);
...
function assign_driver(invoker){
    ...
    $('#addsign_driver_modal input[name="redirect_url"]').val(admin_url+'logistic/shipping');
...
function send_shipping(shipping_id){
    "use strict";
    $('#send_mail_modal_container').empty();
    $("#send_mail_modal_container").load(admin_url + 'logistic/send_shipping_modal/' + shipping_id, function(response, status, xhr) {
          if (status == "error") {
              alert_float('danger', xhr.statusText);
          }
          init_selectpicker();
          $('input[name="redirect_url"]').val(admin_url+'logistic/shipping');
...
/**
 * [approval_pickup description]
 * @return {[type]} [description]
 */
function approval_pickup(el){
    "use strict";
    var shipping_id = $(el).data('shipping_id');

    $('#approval_modal texarea[name="approve_note"]').val('');
    $('#approval_modal input[name="shipping_id"]').val(shipping_id);
    $('#approval_modal input[name="redirect_url"]').val(admin_url+'logistic/shipping_detail/'+shipping_id);

    $('#approval_modal').modal('show');
}

function approve_pickup(type){
    "use strict";
    $('#approval_modal input[name="approval_status"]').val(type);
    
    $('#approval_modal-form').submit();
}
