...
function upload_sign(id, sign_code){
  "use strict";
    ...
    $.post(admin_url + 'logistic/shipping_upload_sign/' + id, data).done(function(response){
...
function remove_sign(id){
  "use strict";
	if (confirm_delete()) {
		$.post(admin_url + 'logistic/shipping_remove_sign/' + id).done(function(response){
	        response = JSON.parse(response); 
	        if (response.success === true || response.success == 'true') {
	            $('#sign_div').html(response.html);
	        }
  ...
  "use strict";
	...
	if(sign_btn.length > 0){
		...
		if(delivery_date != '' && delivered_by != '' && receive_by != '' &&  document.getElementById("file").files.length > 0 ){
			$('#shipment_tracking_shipping-form').submit();
		}else{
